...
select
    ...
    s.precio_vinculante_combustibles::numeric,
    s.fuente,
    s.observaciones
from staging.stg_precios_vinculantes_combustibles s
join datos_maestros.cat_combustibles c
  on upper(trim(s.nombre_combustible)) = upper(trim(c.nombre))
join datos_maestros.cat_unidades_medida u
  on upper(trim(s.nombre_unidad)) = upper(trim(u.codigo))
join datos_maestros.cat_centrales_generacion cg
  on upper(trim(s.nombre_central_generacion)) = upper(trim(cg.nombre));
